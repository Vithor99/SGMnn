...
class SoftActorCritic(nn.Module):
    ...
    def __init__(self, input_dim=2, architecture_params=None, output_dim=2, lr=1e-3, gamma=0.99, epsilon=0.0, batch_size=128, alpha=0, learn_std=True, device=None):
        ...
        self.value_net1 = ValueNetwork(input_dim+output_dim, architecture_params, 1)
        self.value_net2 = ValueNetwork(input_dim+output_dim, architecture_params, 1)
        self.value_net1_target = ValueNetwork(input_dim+output_dim, architecture_params, 1)
        self.value_net2_target = ValueNetwork(input_dim+output_dim, architecture_params, 1)
    ...
    def update(self):
        ...
        rewards = torch.squeeze(torch.from_numpy(rewards).float().to(self.device))
        actions = torch.from_numpy(actions).float().to(self.device)
        terminal = torch.squeeze(torch.from_numpy(terminal).float().to(self.device))


        with torch.no_grad():

            next_actions, next_logprob = self.policy_net.get_action(next_states)
            q1_next = torch.squeeze(self.value_net1_target(next_states, next_actions))
            q2_next = torch.squeeze(self.value_net2_target(next_states, next_actions))
            q_next = torch.min(q1_next, q2_next) - self.log_alpha.exp().detach() * next_logprob
            q_target = rewards + self.gamma * (1 - terminal) * q_next
            # TODO: squeeze?

        q1 = torch.squeeze(self.value_net1(states, actions))
        q2 = torch.squeeze(self.value_net2(states, actions))
        ...
        q1 = torch.squeeze(self.value_net1(states, new_actions))
        q2 = torch.squeeze(self.value_net2(states, new_actions))
        ...
        soft_update(self.value_net2, self.value_net2_target, 0.005)
